...
const MOCK_PROFILES: SearcherProfile[] = [
  {
    ...
    housingPreferences: {
      desiredLocation: ['Milano'],
      moveInDate: new Date('2024-02-01'),
      stayDuration: { min: 6, max: 12 },
      groupSize: 2
    },
    lifestylePreferences: {
      ...
      petFriendly: false,
      additionalTags: ['Okay with Subrenting', 'Non-Smokers']
    },
    metadata: {
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  },
  {
    ...
    academicProfile: {
      ...
      university: 'Università degli Studi di Roma'
    },
    financialInfo: {
      monthlyIncome: 600,
      financialStatus: 'Student',
    ...
    housingPreferences: {
      desiredLocation: ['Roma', 'Napoli'],
      moveInDate: new Date('2024-03-15'),
      stayDuration: { min: 3, max: 9 },
      groupSize: 3
    },
    lifestylePreferences: {
      smokingPreference: 'Non-Smoker',
      alcoholPreference: 'Non-Drinker',
      petFriendly: false,
      additionalTags: ['International Students Welcome', 'LGBTQ+ Friendly']
    },
    metadata: {
      createdAt: new Date(),
      updatedAt: new Date(),
...
export const SearcherProfileList: React.FC = () => {
  ...
  const filteredProfiles = profiles.filter(profile => 
    (profile.financialInfo.budgetPreference.min >= filters.minBudget && 
     profile.financialInfo.budgetPreference.max <= filters.maxBudget) &&
    (filters.locations.length === 0 || 
     profile.housingPreferences.desiredLocation.some(loc => filters.locations.includes(loc)))
  );
              ...
              <div className="p-6">
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800">
                      {profile.personalInfo.contactInfo.telegramUsername}
                    </h3>
                  </div>
                </div>
                ...
                <div className="space-y-3 text-gray-600">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>
                      €{profile.financialInfo.budgetPreference.min} - 
                      €{profile.financialInfo.budgetPreference.max}/month
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FaMapMarkerAlt className="w-5 h-5 text-blue-500" />
                    <span>{profile.housingPreferences.desiredLocation.join(', ')}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FaCalendar className="w-5 h-5 text-purple-500" />
                    <span>
                      {t('searchersPage.moveIn')}: {profile.housingPreferences.moveInDate.toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FaUsers className="w-5 h-5 text-indigo-500" />
                    <span>
                      {profile.housingPreferences.groupSize} {t('searchersPage.people')}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-700">
                      ...
                      {profile.financialInfo.monthlyIncome}€
                    </span>
                  </div>
                </div>

                {profile.lifestylePreferences.additionalTags.length > 0 && (
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {profile.lifestylePreferences.additionalTags.map(tag => (
                        <span 
                          key={tag} 
                ...
                <div className="mt-6">
                  <button 
                    ...
                    onClick={() => {
                      ...
                      window.open(`https://t.me/${profile.personalInfo.contactInfo.telegramUsername.replace('@', '')}`, '_blank');
                    }}
                  >
...
export default SearcherProfileList;